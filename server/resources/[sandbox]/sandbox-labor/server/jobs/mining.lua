						...
						if successful and _mining[_joiners[source]].nodes[lastId] ~= nil then
							...
							if luck >= 95 or node.attempts >= 3 then
								...
								if luck >= 75 then
									...
									Loot.Sets:GemRandom(char:GetData("SID"), 1, loot)
								end
								Loot.Sets:Ore(char:GetData("SID"), 1, math.random(1, 2))
							else
								node.inUse = false
								Inventory:AddItem(char:GetData("SID"), "crushedrock", math.random(7), {}, 1)
							end
