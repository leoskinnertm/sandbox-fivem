...
export default () => {
	...
	const docOnDuty = onDuty.filter((m) => Boolean(m.primary) && m.job === 'prison');
	...
	const doccount = emergencyMembers.filter((m) => Boolean(m.Callsign) && m.Job === 'prison').length;
	...
	return (
		<Grid className={classes.container} container>
						...
						<List className={classes.list}>
							{docOnDuty.length > 0 &&
								docOnDuty
									.sort((a, b) => a.primary - b.primary)
									.map((unit, k) => {
										return <Unit key={`unit-${k}`} unitData={unit} unitType="prison" />;
									})}
						</List>
					) : (
						<List className={classes.list}>
			...
			<Grid item xs={job.Id != 'tow' ? 3 : 12} className={classes.wrapper}>
				<h3>
					Tow
					{towOnDuty.length > 0 && (
						<small>
